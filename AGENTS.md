...
> Instrucciones de proyecto para cualquier agente de IA. Este archivo es la fuente única de
> las reglas del workflow SDD — no las dupliques a mano en otro lado. Los archivos por
> herramienta (`CLAUDE.md`, `GEMINI.md`, `.claude/agents/`, `.cursor/agents/`,
> `.opencode/agent/`, `.kiro/agents/`, `.agents/rules/`) los genera el instalador a partir de
> este mismo contenido: si hay que cambiar una regla, se cambia en el instalador y se vuelve
> a correr, nunca a mano en el destino.

## Stack

- {completar: lenguajes, frameworks, base de datos}

## Comandos

- Instalar: `{comando}`
- Levantar en desarrollo: `{comando}`
- Tests: `{comando}`
- Build: `{comando}`

## Convenciones

- {completar: estilo de commits, naming, estructura de carpetas}

## Dónde vive cada cosa

| Ruta | Qué es |
|---|---|
| `AGENTS.md` (este archivo) | reglas del orquestador SDD — el ciclo, los gates, los formatos |
| `.agents/skills/sdd-{fase}/` | qué hace cada fase. Fuente canónica de las skills |
| `.agents/skills/sds-*/`, `ponytail*/` | skills de dominio y de revisión |
| `sdd/sdd.config.md` | config del proyecto: extensiones activas, slices, umbrales |
| `sdd/{change-name}/` | artifacts de cada cambio en curso |
...
## Mecanismo concreto — Antigravity

Antigravity descubre las skills en `.agents/skills/sdd-{fase}/SKILL.md`, sin configuración
adicional. **Invocá cada fase explícitamente** por su nombre de skill ("usá la skill
`sdd-{fase}`"). No te apoyes en la activación implícita para conducir el ciclo: las
`description` de las skills describen el trabajo de cada fase, no el arranque del workflow, y
un pedido como "agregá X" no dispara ninguna por sí solo.

Estas reglas te llegan siempre, por dos vías: `.agents/rules/orchestrator.md`, que Antigravity
auto-inyecta en la sesión, y el `AGENTS.md` de la raíz del proyecto. No hay un paso extra que
tengas que dar para "cargar" el protocolo — si estás leyendo esto, ya está activo.

Antigravity **no tiene sub-agentes con contexto aislado** para este flujo: corrés el ciclo en
línea, una fase por vez, según el default de "Cuándo arranca el ciclo". Todas las fases
comparten tu ventana de contexto, así que al avanzar de una a la siguiente sé explícito sobre
**qué artifacts leer** (rutas concretas bajo `sdd/{change-name}/`) y cerrá cada fase con su
artifact escrito antes de empezar la próxima, en vez de asumir que la conversación previa
alcanza como contexto.

Antigravity también tiene Workflows (`.agents/workflows/*.md`, invocados con `/nombre`). Este
protocolo no los usa: las Skills son el mecanismo que comparte con Gemini CLI, Codex CLI y
OpenCode sin ninguna copia adicional. Si querés atajos `/sdd-{fase}` propios, agregalos como
Workflows opcionales sobre la misma skill — no reemplaces la skill con un Workflow.
