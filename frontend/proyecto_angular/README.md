...
Proyecto Angular 21 (standalone, signals) para la gestión multi-empresa de arriendos. Incluye
el sitio público de marketing de KUVU (`/`, `/nosotros`, `/preguntas-frecuentes`) y la app
autenticada de gestión.

---

## 📋 Descripción

Aplicación web que permite a múltiples empresas inmobiliarias (Amarilo, Nido Rent, Balcones de San Soucci, Mi Inmueble) gestionar de forma independiente sus locales, contratos, pagos y mantenimientos. El sitio público de marketing comparte chrome (header/footer, tema claro/oscuro) y su contenido vive centralizado en `src/app/content/`.
...
> 1. Al cargar la app aparece una intro de ~5s (una vez por carga) en `/acceder`; seleccionar una
>    empresa y luego ingresar el correo y el número de documento correspondientes a esa empresa.
...
```bash
# Correr la suite (Karma + ChromeHeadless). En Windows, fijar CHROME_BIN antes.
...
src/
└── app/
    ├── content/                       # Textos del sitio público (módulos TS tipados)
    ...
    │   ├── intro-state.service.ts    # Flag de intro "una vez por carga" + reduced-motion
    │   ├── theme.service.ts          # Tema claro/oscuro del sitio público
    │   └── page-meta.service.ts      # title + meta description por página
    │
    ├── directives/
    │   └── reveal.directive.ts       # Reveals [data-reveal] del sitio público
    │
    ├── guards/
    ...
    │   │
    │   ├── landing/                  # Selección de inmobiliaria (/acceder, orquesta la intro)
    │   ├── login/                    # Credenciales de la empresa seleccionada
...
| @angular/forms | 21.x | Formularios reactivos y template-driven |
| @angular/common | 21.x | Pipes, directivas comunes |
| Tailwind CSS | 4.x | Estilos del sitio público (tokens en `@theme`) |
...
*Desarrollado con Angular 21 · Proyecto DSS · 2025*
